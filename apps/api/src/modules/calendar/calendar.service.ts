...
export class CalendarService {
  ...
  async getMonth(year: number, month: number): Promise<CalendarItem[]> {
    ...
    const [noticias, reportajes, guias, places, events, planazoGuides, alertas, lamiraEventos, lamiraLugares] = await Promise.all([
      this.db
        .select({ id: schema.noticias.id, title: schema.noticias.title, slug: schema.noticias.slug, date: schema.noticias.publishedAt })
      ...
      this.db
        .select({ id: schema.reportajes.id, title: schema.reportajes.title, slug: schema.reportajes.slug, date: schema.reportajes.publishedAt })
      ...
      this.db
        .select({ id: schema.guias.id, title: schema.guias.title, slug: schema.guias.slug, date: schema.guias.createdAt })
        .from(schema.guias)
        .where(and(eq(schema.guias.status, 'published'), gte(schema.guias.createdAt, start), lt(schema.guias.createdAt, end))),
      this.db
        .select({ id: schema.places.id, title: schema.places.name, slug: schema.places.slug, date: schema.places.createdAt })
        .from(schema.places)
        .where(and(eq(schema.places.status, 'published'), gte(schema.places.createdAt, start), lt(schema.places.createdAt, end))),
      this.db
        .select({ id: schema.events.id, title: schema.events.name, slug: schema.events.slug, date: schema.events.createdAt })
        .from(schema.events)
        .where(and(eq(schema.events.status, 'published'), gte(schema.events.createdAt, start), lt(schema.events.createdAt, end))),
      this.db
        .select({ id: schema.planazoGuides.id, title: schema.planazoGuides.title, slug: schema.planazoGuides.slug, date: schema.planazoGuides.createdAt })
      ...
      this.db
        .select({ id: schema.alertas.id, title: schema.alertas.title, slug: schema.alertas.slug, date: schema.alertas.createdAt })
        .from(schema.alertas)
        .where(and(gte(schema.alertas.createdAt, start), lt(schema.alertas.createdAt, end))),
      this.db
        .select({ id: schema.lamiraEventos.id, title: schema.lamiraEventos.title, slug: schema.lamiraEventos.slug, date: schema.lamiraEventos.createdAt })
        .from(schema.lamiraEventos)
        .where(and(gte(schema.lamiraEventos.createdAt, start), lt(schema.lamiraEventos.createdAt, end))),
      this.db
        .select({ id: schema.lamiraLugares.id, title: schema.lamiraLugares.name, slug: schema.lamiraLugares.slug, date: schema.lamiraLugares.createdAt })
        .from(schema.lamiraLugares)
        .where(and(gte(schema.lamiraLugares.createdAt, start), lt(schema.lamiraLugares.createdAt, end))),
    ]);

    const tag = (rows: { id: string; title: string; slug: string; date: Date }[], contentType: string, site: 'la-mira' | 'planazo'): CalendarItem[] =>
      rows.map((r) => ({ contentType, contentId: r.id, title: r.title, slug: r.slug, site, date: r.date.toISOString() }));
